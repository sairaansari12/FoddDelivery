...
var promoCodeApplied=""
  let responseNull=  commonMethods.checkParameterMissing([params.serviceDateTime,params.addressId])
     ...
     if(findData)
{
  ...
  PAYMENT.destroy({where:{orderId: findData.dataValues.orderId}})
   ...
   //else{
    ...
    var schedules = await SCHEDULE.findOne({where :{companyId: req.companyId,dayParts:dayCount}})
...
const coupanDetails = await COUPAN.findOne({
 ...
 //const cOrderData=null
  ...
  {
    paymentEntry(req.id,req.companyId,cOrderData.dataValues.id,totalPrice)
    subtractBookingsCount(slotdata,countDataq.totalQuantity,params.serviceDateTime,dayCount,req.companyId)
